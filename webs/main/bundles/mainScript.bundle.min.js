...
/* countDown js */
var countDownTimer,countDownDate=new Date("Sep 25, 2025 15:00:00").getTime();function stopCountDown(){clearInterval(countDownTimer)}function startCountDown(t){stopCountDown();let n=1e3*hmsToSecondsOnly(t);sendPacket({type:"countDown",time:n}),setCountDownTime(n),countdownTimeStart(),$(".swOffBtn").addClass("runningCountDown")}function setCountDownTime(t){countDownDate=(new Date).getTime()+t}function countdownTimeStart(){countDownTimer=setInterval(function(){var t=(new Date).getTime(),n=countDownDate-t,e=Math.floor(n%864e5/36e5),o=Math.floor(n%36e5/6e4),i=Math.floor(n%6e4/1e3);e<10&&(e="0"+e),o<10&&(o="0"+o),i<10&&(i="0"+i),$("#hourTimer").text(e),$("#minTimer").text(o),$("#secTimer").text(i),n<0&&(clearInterval(countDownTimer),console.log("Timer hit!"))},1e3)}
/* countDown js */

/* timepicker js */
function initTimePickers(){initInlinePicker()}function hmsToSecondsOnly(e){for(var n=e.split(":"),t=0,i=1;n.length>0;)t+=i*parseInt(n.pop(),10),i*=60;return t}var swOffPicker;function initInlinePicker(){swOffPicker=new Picker(document.querySelector(".swOffBtn"),{format:"HH:mm",date:new Date(0,20),controls:!0,text:{title:"Visszaszámláló",cancel:"Mégsem",confirm:"OK",year:"Év",month:"Hónap",day:"Nap",hour:"Óra",minute:"Perc",second:"Másodperc",millisecond:"Ms"},pick:function(e){startCountDown(swOffPicker.getDate(!0)+":00")}})}
/* timepicker js */
